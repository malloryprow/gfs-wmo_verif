...
my @n;
 ...
 $infile_sal1l2 = $data . '/agg_stat_SAL1L2_to_CNT.sta.' . $yyyy . $mm . $dd . '.' . $hh . 'Z';
 $infile_sl1l2 =  $data . '/agg_stat_SL1L2_to_CNT.sta.'  . $yyyy . $mm . $dd . '.' . $hh . 'Z';
 $infile_sl1l2_noAC = $data . '/agg_stat_SL1L2_to_CNT.sta.' . $yyyy . $mm . $dd . '.' . $hh . 'Z';
 $infile_s1_mslp = $data . '/agg_stat_GRAD_sta.' . $yyyy . $mm . $dd . '.' . $hh . 'Z';
 $infile_vl1l2 =  $data . '/agg_stat_VL1L2_to_VCNT.sta.'  . $yyyy . $mm . $dd . '.' . $hh . 'Z';
...
for ( $v=0; $v<=0; $v++) {
    for ( $d=0; $d<=$#domain; $d++) {
      for ( $f=0; $f<=$#fhr; $f++) {
          ...
          if ( $v == 0 ) { # mslp
            #Binbin Z: 20210325, modify for request from CMC
...
print "centre=kwbc,ref=an,model=GFS,d=$yyyy$mm$dd,t=$hh, ";
...
for ($d=0; $d<=$#domain; $d++) { 
  ...
  for ($s=$#score_wmo_mslp; $s<=$#score_wmo_mslp; $s++) {
   ...
   print "dom=$dom_wmo[$d],par=mslp,sc=$score_wmo_mslp[$s],";
    #Binbin Z: 20210325 modify for request from CMC not /100, so keep 1 digit behind point
...
# Other variables: only has-SL1L2 variables (i.e. only noAC varibles)
